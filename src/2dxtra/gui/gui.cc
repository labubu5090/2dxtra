...
namespace iidxtra::gui
{
    ...
    auto wndproc_hwnd = static_cast<HWND>(nullptr);
    ...
    auto begin() -> void
    {
        ...
        {
            // Keyboard keys map to button bits as key[n] -> bit n-1 (0-based)
            // on this build. A runtime test with bit1/bit2 on Up/Down showed
            // the working keys as [2]/[3] (one past the hint), so 0-based is
            // correct: [1] is bit 0, [6] is bit 5, P2 starts at bit 7.
            ...
            // The game does not deliver normal Win32 mouse messages on this
            // build, so feed the mouse from the OS cursor directly.
            if (auto* const hwnd = wndproc_hwnd; hwnd != nullptr)
            {
                auto rect = RECT {};
                if (GetClientRect(hwnd, &rect) && rect.right > 0 && rect.bottom > 0)
                {
                    ...
                    {
                        auto const scale = ImVec2(
                            io.DisplaySize.x / static_cast<float>(rect.right),
                            io.DisplaySize.y / static_cast<float>(rect.bottom));
                        io.AddMousePosEvent(pos.x * scale.x, pos.y * scale.y);
                    }
                }

                auto static last_lmb = false;
                auto const lmb = (GetAsyncKeyState(VK_LBUTTON) & 0x8000) != 0;
                if (lmb != last_lmb)
                {
