...
namespace iidxtra::gui::timing_modifier_window
{
	...
	auto render() -> void
    {
        ...
        if (bm2dx::state && bm2dx::state->play_style == 0 && !bm2dx::state->p1_active && !bm2dx::state->p2_active)
        {
        ...
        auto const play_style = (bm2dx::state ? bm2dx::state->play_style : 0);
        auto const p1_active = (bm2dx::state ? bm2dx::state->p1_active : 1);
        auto const p2_active = (bm2dx::state ? bm2dx::state->p2_active : 0);

        if (p1_active || play_style == 1) draw_player_config_tab(0);
        if (p2_active || play_style == 1) draw_player_config_tab(1);
