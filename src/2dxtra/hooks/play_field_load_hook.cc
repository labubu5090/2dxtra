...
namespace iidxtra::play_field_load_hook
{
	...
	auto play_field_load_hook_fn(bm2dx::play_field_t* play_field, void* a2, void* a3, int a4) -> void*
	{
        ...
        if (bm2dx::state && (bm2dx::state->p1_active || bm2dx::state->play_style == 1)) cn_override::set_initial_states(0);
        if (bm2dx::state && (bm2dx::state->p2_active || bm2dx::state->play_style == 1)) cn_override::set_initial_states(1);
	...
	{
		if (bm2dx::addr->PLAY_FIELD_LOAD != nullptr)
